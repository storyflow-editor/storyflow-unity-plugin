...
- **Map variables**: full support for the editor's map variable type. Maps preserve insertion order, support any key and value type combination and can be declared on projects, scripts and characters. The importer parses map variables and their default entries from synced script, project and character data
...
- **Map character variables**: `setCharacterVar` can now write map-typed character variables. The character stores an independent copy of the wired map, and writes broadcast both `OnVariableChanged` and `OnCharacterVariableChanged`
...
- **Map save/load**: map variables on globals and characters round-trip through save slots, including entry order. The save format version is unchanged and existing saves load exactly as before
